import os
# Mandatory Vercel serverless configuration for matplotlib
...
def fetch_arxiv_literature(query: str, max_results: int = 8) -> tuple[str, str]:
    ...
    try:
        ...
        for i, entry in enumerate(root.findall('atom:entry', ns)):
            ...
            title = title_el.text.replace('\n', ' ').strip() if title_el is not None else "Unknown Title"
            summary = summary_el.text.replace('\n', ' ').strip() if summary_el is not None else ""
            published = published_el.text[:4] if published_el is not None else "2024"
            ...
            first_word = re.sub(r'[^a-zA-Z0-9]', '', title.split()[0].lower()) if title else "paper"
        ...
        print(f"arXiv API fetch failed: {e}")
        return "", ""
...
def call_gemini_with_retry(api_key: str, prompt: str, retries: int = 3) -> str:
    """Executes Gemini API calls with exponential backoff."""
...
def clean_llm_latex_output(raw_output: str) -> str:
    """Removes structural markdown code-fences."""
...
def generate_academic_chart(api_key: str, paper_title: str, results_notes: str, figure_index: int) -> tuple[str, bytes] | None:
    client = genai.Client(api_key=api_key)
    # Using /tmp ensures writable permissions in Serverless environments
    chart_filename = f"/tmp/generated_chart_{figure_index}_{int(time.time())}.png"
    ...
    try:
        ...
        if os.path.exists(chart_filename):
            ...
            plt.close('all')
            return os.path.basename(chart_filename), img_bytes
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
        ...
        if uploaded_figs:
            for uf in uploaded_figs:
                # uf is a wrapper around FileStorage containing name and bytes
...
def compile_pdf(latex_src: str, fig_bins: list[tuple[str, bytes]] | None = None) -> tuple[bytes | None, str | None]:
    """Compiles PDF via pdflatex. Handled gracefully if unavailable in Vercel."""
    with tempfile.TemporaryDirectory() as tmp:
        ...
        cmd = ["pdflatex","-interaction=nonstopmode","-halt-on-error", "-output-directory", tmp, str(tex)]
        try:
            for _ in range(2):
                r = subprocess.run(cmd, capture_output=True, text=True, cwd=tmp, timeout=120)
        except FileNotFoundError:
            # Note: Serverless environments like Vercel usually do not have pdflatex installed natively.
            # This is expected behavior and will default gracefully to ZIP/TeX source downloads.
...
def api_generate():
    try:
        # Extract Text Data
        api_key = request.form.get("api_key", "").strip() or os.getenv("GEMINI_API_KEY")
        if not api_key:
        ...
        if authors_df.empty or "Name" not in authors_df.columns:
             return jsonify({"success": False, "error": "At least one author is required."}), 400
        ...
        # Extract Files
        raw_files = request.files.getlist("uploaded_figs")
        uploaded_figs = []
        for rf in raw_files:
            if rf.filename:
                uploaded_figs.append({"name": rf.filename, "data": rf.read()})

        # Pipeline Initialization
        sec_targets = compute_section_targets(target_pages, two_col)
        
        # Step 1: ArXiv Grounding
        ...
        zip_payload_bytes = generate_overleaf_zip(latex_src, combined_bibtex, uploaded_figs, generated_charts_list)
        zip_b64 = base64.b64encode(zip_payload_bytes).decode('utf-8')

        # Optional Step 5: Try PDF
        pdf_bytes, pdf_err = compile_pdf(latex_src, generated_charts_list + [(uf['name'], uf['data']) for uf in uploaded_figs])
